...
import project.shop.user.dto.response.QUserResponse;
...
import static project.shop.user.entity.QUser.user;
...
public class UserRepositoryImpl implements UserRepositoryCustom {
    ...
    @Override
    public Page<UserResponse> findAll(UserSearchCondition condition, Pageable pageable) {
    ...
    private BooleanExpression withdrawAtBetween(LocalDateTime from, LocalDateTime to) {
        if (from == null && to == null) return null;
        if (from != null && to != null) return user.withdrawAt.between(from, to);
        if (from != null) return user.withdrawAt.goe(from);
        return user.withdrawAt.loe(to);
    }

    private BooleanExpression lastLoginAtBetween(LocalDateTime from, LocalDateTime to) {
        if (from == null && to == null) return null;
        if (from != null && to != null) return user.lastLoginAt.between(from, to);
        if (from != null) return user.lastLoginAt.goe(from);
